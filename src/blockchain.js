...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async(resolve, reject) => {
            let newHeight = self.height + 1//self.chain.length();
            ...
            if(block.hash){
                ...
                reject('error generating has to be added to the chain');
            }
            ...
            if(timeDiff/60 > 5){
              ...
              if(bitcoinMessage.verify(message, address, signature)){
                ...
                resolve(newblock);
                 ...
                 reject('wallet address not verified');
              }
            }
        });
    }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise(async (resolve, reject) => {
           let block = await self.chain.filter((c)=>{
               return c.hash == hash;
            })
            if(!block){
    ...
    getStarsByWalletAddress (address) {
        ...
        let stars = [];
        return new Promise((resolve, reject) => {
          self.chain.forEach(async(bdata) => {
             let blockdata = await bdata.getBData();
